(ns kudamori.methods.social
  "social.cljc — kudamori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kudamori's own
...
(defn draft-observation-post
  "A dry-run observation post over kudamori's own domain (Sewer / confined-space in-pipe cleaning robotics — removes the human from the confined space that ADR-2606073001 §4 named as a…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kudamori R0: live social posting is Council Lv6+ + operator + "
