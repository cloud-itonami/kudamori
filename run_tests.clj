#!/usr/bin/env bb
;; kudamori — bb-native test runner (Clojure / babashka; no shell). Auto-generated
...
(def suites
  '[kudamori.methods.test-kudamori])

(apply require suites)

(let [{:keys [fail error]} (apply t/run-tests suites)]
  (if (zero? (+ fail error))
    (println "-- kudamori: ALL suites green --")
    (do (println "-- kudamori: FAILURES above --")
        (System/exit 1))))
